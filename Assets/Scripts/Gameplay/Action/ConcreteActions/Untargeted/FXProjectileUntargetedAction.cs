...
namespace Unity.BossRoom.Gameplay.Actions
{
    ...
    [CreateAssetMenu(menuName = "BossRoom/Actions/FX Projectile Untargeted Action")]
    public partial class FXProjectileUntargetedAction : Action
    {
        ...
        public override bool OnStart(ServerCharacter serverCharacter)
        {
            ...
            //Vector3 firePos = serverCharacter.physicsWrapper.Transform.position + new Vector3(0,1,0);
            Vector3 firePos = serverCharacter.physicsWrapper.Transform.position;//+ Config.FirePosition
            Transform firePosTransform = serverCharacter.physicsWrapper.Transform;
            firePos += firePosTransform.forward * Config.FirePosition.z + firePosTransform.right * Config.FirePosition.x + firePosTransform.up * Config.FirePosition.y;
            Vector3 fireDirection = serverCharacter.physicsWrapper.Transform.forward; //+ serverCharacter.physicsWrapper.Transform.right * UnityEngine.Random.Range(-Config.Radius, Config.Radius);

            if (DetectTarget(firePos, fireDirection, Config.Projectiles[0].Range, out Vector3 targetPos,
                    out GameObject target))
            {
                m_DamageableTarget = target.GetComponent<IDamageable>();
                Data.Position = targetPos;
            }
            else
            {
                Data.Position = firePos + fireDirection.normalized * Config.Projectiles[0].Range;
            }
            ...
            float distanceToTargetPos = Vector3.Distance(targetPos, firePos);
        ...
        private bool DetectTarget(Vector3 characterPos, Vector3 direction, float maxDistance, out Vector3 targetPosition, out GameObject target)
        {
            RaycastHit hit;
            if (Physics.Raycast(characterPos, direction, out hit, maxDistance))
            {
