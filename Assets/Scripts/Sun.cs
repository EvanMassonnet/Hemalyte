...
using UnityEngine;
    ...
    public class Sun : MonoBehaviour
    {
        ...
        void SetPosition()
        {
            ...
            double alt;
            double azi;
            SunPosition.CalculateSunPosition(clock.worldDate, (double)latitude, (double)longitude, out azi, out alt);
            angles.x = (float)alt * Mathf.Rad2Deg;
            angles.y = (float)azi * Mathf.Rad2Deg;
            //UnityEngine.Debug.Log(angles);
            transform.localRotation = Quaternion.Euler(angles);
            light.intensity = Mathf.InverseLerp(-12, 0, angles.x) * lightIntensity;
            //Debug.Log(clock.worldDate + "     " + Mathf.InverseLerp(-12, 0, angles.x));
        }
